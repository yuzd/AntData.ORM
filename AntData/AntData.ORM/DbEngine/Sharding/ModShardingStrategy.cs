...
namespace AntData.ORM.DbEngine.Sharding
{
    class ModShardingStrategy : IShardingStrategy
    {
        ...
        private static IComparable GetShardColumnValueByParameters(String logicDbName, IList<String> shardColumns, StatementParameterCollection parameters, Boolean quote = false)
        {
            IComparable shardColumnValue = null;
            ...
            foreach (var item in shardColumns)
            {
                String name = quote ? item.ToLower() : item.ToLower();
